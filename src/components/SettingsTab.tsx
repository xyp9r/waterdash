...
export default function SettingsTab({ currentGoal, profile, onUpdateProfile, favoriteDrinks, onDeleteFavorite }: SettingsTabProps) {
	...
	return (
		<div className="space-y-6 relative h-full pb-20">
			<h2 className="text-2xl font-bold text-slate-500 mb-6 px-2 dark:text-slate-100">Profile & Settings</h2>

			{/* Карточка юзера */}
			<div className="rounded-3xl p-5 bg-white border border-slate-100 shadow-sm dark:bg-slate-700/10 dark:border-slate-900">
				<div className="flex items-center gap-4">
					...
					<div>
							<h3 className="text-slate-600 text-lg font-bold dark:text-slate-200">{profile.name}</h3>
			...
			{/* Список настроек */}
			<div className="rounded-3xl overflow-hidden bg-white border border-slate-100 shadow-sm dark:bg-slate-700/10 dark:border-slate-900">
			...
			<button
				...
				dark:bg-slate-700/10 hover:bg-slate-50 dark:border-slate-900 dark:hover:bg-slate-800/10 dark:text-slate-300">
			...
			{favoriteDrinks.length > 0 && (
					<div className="rounded-3xl p-5 bg-white border border-slate-100 shadow-sm dark:bg-slate-700/10 dark:border-slate-900">
						<h3 className="text-slate-500 font-medium mb-4 px-2 dark:text-slate-300">Saved Presets</h3>
						<div className="grid grid-cols-2 gap-3">
							{favoriteDrinks.map(drink => (
									<div key={drink.id} className="bg-slate-50 rounded-2xl py-3 flex items-center justify-between border border-slate-100 shadow-sm dark:bg-slate-700/10 dark:border-slate-900 dark:hover:bg-slate-800/10 dark:text-slate-300">
										<div className="flex items-center gap-3">
											<span className="text-2xl pl-3">{drink.icon}</span>
											<div className="flex flex-col">
												<span className="text-slate-500 font-bold text-sm dark:text-slate-300">{drink.amount} ml</span>
			...
			{activeModal && (
				<>
					...
					<div className="
						...
						<h3 className="text-slate-500 text-2xl font-bold text-center mb-8 capitalize dark:text-slate-300">Update {activeModal}</h3>
						{activeModal === 'gender' && (
								<div className="grid grid-cols-2 gap-4 mb-8">
									<button onClick={() => handleSave('male')} className={`py-4 rounded-2xl font-bold transition-all ${profile.gender === 'male' ? 'dark:bg-blue-500/10 dark:text-blue-400 dark:border-blue-500/50 bg-white text-slate-400 shadow-lg shadow-blue-500/30 border border-slate-400' : 'dark:bg-transparent dark:text-slate-400 dark:border-slate-700 dark:hover:bg-slate-800 dark:hover:text-slate-300 bg-blue-100 text-slate-400 border border-slate-300'}`}>Male</button>
									<button onClick={() => handleSave('female')} className={`py-4 rounded-2xl font-bold transition-all ${profile.gender === 'female' ? 'dark:bg-blue-500/10 dark:text-blue-400 dark:border-blue-500/50 bg-white text-slate-400 shadow-lg shadow-blue-500/30 border border-slate-400' : 'dark:bg-transparent dark:text-slate-400 dark:border-slate-700 dark:hover:bg-slate-800 dark:hover:text-slate-300 bg-blue-100 text-slate-400 border border-slate-300'}`}>Female</button>
								</div>
							)}
						{(activeModal === 'weight' || activeModal === 'height' || activeModal === 'goal') && (
								<div className="space-y-6">
										<input type="number" autoFocus value={tempValue} onChange={(e) => setTempValue(e.target.value)} className="w-full bg-slate-100 text-slate-600 text-4xl font-bold text-center py-6 rounded-3xl border-2 border-slate-300 focus:border-blue-400 outline-none transition-all dark:bg-transparent dark:text-slate-400 dark:border-slate-700 dark:focus:text-slate-200" />
										<button onClick={() => handleSave()} className="w-full bg-blue-50 dark:bg-slate-900/10 dark:text-slate-200 dark:border-slate-900 text-slate-500 py-4 rounded-2xl font-bold text-lg shadow-lg shadow-blue-500/20 active:scale-95 transition-all border border-slate-300">Save Changes</button>
								</div>
							)}
...
function SettingItem({ label, value, onClick, highlight = false }: any) {
	return (
			<button onClick={onClick} className="flex justify-between items-center w-full p-5 hover:bg-slate-50 transition-colors border-b border-slate-300 last:border-0 dark:border-slate-900 dark:hover:bg-slate-700/10">
				<span className="text-slate-500 font-medium dark:text-slate-300">{label}</span>
				<span className={`${highlight ? 'text-blue-400 font-bold' : 'text-slate-500 font-medium dark:text-slate-400'}`}>
					{value} <span className="ml-1 opacity-30 dark:text-slate-300">{`➔`}</span>
				</span>
			</button>
		);
}